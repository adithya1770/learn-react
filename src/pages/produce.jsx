    ...
    return(
        <div className="poppins-bold">
          <p class="text-8xl font-normal pl-6 tracking-wide text-white poppins-bold absolute top-4 antialiased">Farmy</p>
            ...
            <input type="text" placeholder="Produce Name" onChange={(e) => {setproduceName(e.target.value)}} className="easer placeholder-black lg:w-64 w-80 ml-7 lg:h-10 text-2xl rounded-3xl text-center border-2 border-black hover:shadow-2xl"/><br /><br />
            <input type="text" placeholder="Produce Cost" onChange={(e) => {setproduceCost(e.target.value)}} className="easer placeholder-black lg:w-64 w-80 ml-7 lg:h-10 text-2xl rounded-3xl text-center border-2 border-black hover:shadow-2xl"/><br /><br />
            <input type="text" placeholder="Seller Name" onChange={(e) => {setsellerName(e.target.value)}} className="easer placeholder-black lg:w-64 w-80 ml-7 lg:h-10 text-2xl rounded-3xl text-center border-2 border-black hover:shadow-2xl" /><br /><br />
            <input type="text" placeholder="Quantity (Kgs)" onChange={(e) => {setquantity(e.target.value)}} className="easer placeholder-black lg:w-64 w-80 ml-7 lg:h-10 text-2xl rounded-3xl text-center border-2 border-black hover:shadow-2xl"/><br /><br />
            <button onClick={prod} className="text-3xl bg-black lg:h-10 lg:w-64 text-white rounded-3xl text-center easer absolute lg:left-96 lg:top-80 left-24 border-2 border-white hover:text-black hover:bg-white w-48">Publish</button><br />
            </div><br />
            <button onClick={downloadProd} className="text-3xl bg-black lg:h-10 lg:w-64 text-white rounded-3xl text-center easer  absolute lg:left-96 lg:top-96 top-100 left-16 border-2 border-white hover:text-black hover:bg-white w-64">View Details</button>
