    ...
    return(
        ...
        <img src="https://cdn-icons-png.flaticon.com/512/732/732090.png" alt="https://w7.pngwing.com/pngs/1022/900/png-transparent-open-opensource-source-logos-and-brands-line-filled-icon.png" className="h-10 w-10 absolute lg:right-52 lg:top-14 lg:opacity-100 opacity-0" />
          ...
          <p className="text-white lg:text-8xl text-5xl shadow-trans">Transaction</p><br />
            <center>
                <input type="text" placeholder="Price" className="placeholder-black lg:w-64 sm:w-full easer h-10 lg:mb-4 mb-1 lg:text-2xl sm:text-xl rounded-3xl text-center border-2 border-black hover:shadow-2xl" onChange={(e) => { setTransaction(e.target.value) }} /> <br />
                <input type="phone" placeholder="Account Number" className="placeholder-black lg:w-64 easer sm:w-full h-10 lg:text-2xl sm:text-xl rounded-3xl text-center border-2 border-black hover:shadow-2xl" onChange={(e) => { setaccNum(e.target.value) }} /> <br />
                <input type="date" className="placeholder-black lg:w-64 sm:w-full lg:mt-4 mt-1 h-10 easer rounded-3xl text-center lg:text-2xl sm:text-xl border-2 border-black hover:shadow-2xl" onChange={(e) => { setDate(e.target.value) }} /> <br />
                <button onClick={tranSact} className="bg-black  lg:mt-4 mt-1 h-10 w-32 text-white easer rounded-3xl border-2 border-white hover:text-black hover:bg-white easer ">Record</button>
            </center>
            <br />
            <div className="lg:h-100 h-auto lg:w-96 w-full bg-black lg:bg-opacity-100 bg-opacity-15 text-white absolute lg:top-52 top-100 lg:left-2 left-0 text-sm rounded-3xl lg:p-4 p-0">
                <center>
                    ...
                    <input type="text" placeholder="Account Number" className="placeholder-black easer  text-black lg:w-64 sm:w-full h-10 lg:text-2xl sm:text-xl rounded-3xl text-center" onChange={(e) => { setaccNumber(e.target.value) }} /> <br /><br />
                    <button onClick={detail} className="bg-black h-8 w-32 text-white lg:text-xl sm:text-lg easer rounded-3xl border-2 border-white hover:text-black hover:bg-white easer ">Retrieve</button>
                    <button onClick={downloadInfo} className="bg-black h-8 w-32 text-white lg:text-xl sm:text-lg easer rounded-3xl border-2 border-white hover:text-black hover:bg-white easer ">Download</button>
